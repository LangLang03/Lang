...
std::uint32_t load32(const std::vector<std::uint8_t>& data, std::size_t offset) {
...
void store64(std::vector<std::uint8_t>& data, std::uint64_t value) {
    for (int i = 7; i >= 0; --i) {
        data.push_back(static_cast<std::uint8_t>((value >> (i * 8)) & 0xffU));
    }
}
