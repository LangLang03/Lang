...
// linux_arm32 使用大端编码（Big-Endian），与现有 bytecode_binary.cc 的小端编码不同。
...
// linux_arm32 字节码格式（大端编码，字段顺序与现有 bytecode_binary.cc 不同）：
// [0..3]   4 字节魔数
// [4]      1 字节 platform id
// [5..6]   2 字节 version (big-endian)
// [7..8]   2 字节 flags (big-endian)
// [9]      1 字节字节序标签
// [10..]   entry -> env_fingerprint -> producer -> functions
// 与现有格式差异：entry 提前到 byte_order 之后，env_fingerprint 和 producer 顺序对调，
// 且所有多字节整数使用大端编码。
