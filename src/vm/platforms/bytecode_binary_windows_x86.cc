...
// windows_x86 函数字段顺序：name -> returnable -> maxStackSize -> securityLevel -> ...
...
std::optional<FunctionBytecode> ReadFunction(BytecodeReader& reader) {
  ...
  if (!name || !returnable || !max_stack_size || !security_level || !category || !hash || !grantor) {
    return std::nullopt;
  }
...
// windows_x86 序列化格式（小端编码）：
// 头部字段顺序：magic(4) -> flags(2) -> version(2) -> platform_id(1) -> byte_order(1)
...
std::optional<BytecodeProgram> readBytecode(std::istream& input, const std::string& name, Diagnostics& diagnostics) {
  ...
  if (!producer || !entry || !fingerprint || !function_count) {
    return std::nullopt;
  }
