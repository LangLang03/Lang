...
class BytecodeWriter {
 public:
  ...
  void WriteStringVector(const std::vector<std::string>& values) {
    WriteLittleEndian(CheckedCount(values.size(), field_name::kStringVector));
...
class BytecodeReader {
 public:
  ...
  std::optional<std::string> ReadString(std::string_view field) {
    const auto size = ReadLittleEndian<std::uint32_t>(field);
    if (!size) {
      return std::nullopt;
    }
  ...
  bool ReadStringVector(std::vector<std::string>& output, std::string_view field) {
    const auto count = ReadLittleEndian<std::uint32_t>(field);
    if (!count) {
      return false;
    }
...
void WriteFunction(BytecodeWriter& writer, const FunctionBytecode& function) {
  ...
  writer.WriteLittleEndian(CheckedNonNegative(function.maxStackSize, kMaxStackLabel));
  writer.WriteLittleEndian(CheckedNonNegative(function.securityLevel, kSecurityLevelLabel));
  writer.WriteLittleEndian(function.returnable ? bytecode_format::kBooleanTrue : bytecode_format::kBooleanFalse);
  ...
  writer.WriteLittleEndian(CheckedCount(function.code.size(), kInstructionVectorLabel));
...
std::optional<FunctionBytecode> ReadFunction(BytecodeReader& reader) {
  ...
  auto max_stack_size = reader.ReadLittleEndian<std::uint32_t>(field_name::kFunctionMaxStack);
  auto security_level = reader.ReadLittleEndian<std::uint32_t>(field_name::kFunctionSecurityLevel);
  auto returnable = reader.ReadLittleEndian<std::uint32_t>(field_name::kFunctionReturnable);
  ...
  const auto instruction_count = reader.ReadLittleEndian<std::uint32_t>(field_name::kInstructionCount);
...
// android-x86 序列化格式（小端编码）：
// [0..3]   4 字节魔数
// [4..5]   2 字节 version (little-endian)
// [6..7]   2 字节 flags (little-endian)
// [8]      1 字节 platform id
// [9]      1 字节字节序标签
// [10..]   environmentFingerprint / entry / producer / functions
// 注意：与现有 bytecode_binary.cc 的字段顺序不同（version/flags 在 platform_id 之前，
//       entry 和 producer 的顺序也交换了）。
...
std::optional<BytecodeProgram> readBytecode(std::istream& input, const std::string& name, Diagnostics& diagnostics) {
  ...
  // android-x86 字段顺序：version -> flags -> platform_id -> byte_order
  ...
  // android-x86 字段顺序：env_fingerprint -> entry -> producer -> functions
  ...
  if (!fingerprint || !entry || !producer || !function_count) {
    return std::nullopt;
  }
