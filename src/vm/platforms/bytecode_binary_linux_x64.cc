...
// linux_x64 序列化使用小端编码，字段顺序与现有 bytecode_binary.cc 不同：
// magic(4) -> platform_id(1) -> version(2 LE) -> flags(2 LE) -> byte_order(1)
// -> entry -> producer -> env_fingerprint -> functions -> 尾部SHA256(64 字符十六进制)
 ...
 public:
  ...
  void WriteLittleEndian(Integer value) {
    ...
    for (std::size_t byte_index = 0; byte_index < sizeof(Integer); ++byte_index) {
      buffer_.push_back(static_cast<char>(remaining & kByteMask));
      remaining >>= kByteBits;
    }
  }
...
std::optional<BytecodeProgram> readBytecode(std::istream& input, const std::string& name, Diagnostics& diagnostics) {
  ...
  // 字段顺序与现有 bytecode_binary.cc 不同：entry -> producer -> env_fingerprint
