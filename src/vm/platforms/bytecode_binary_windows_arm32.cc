// windows-arm32 平台字节码序列化实现
// 使用大端编码，字段顺序与现有 bytecode_binary.cc 不同：
// magic(4) -> byte_order(1) -> flags(2 BE) -> platform_id(1) -> version(2 BE)
//        -> functions -> entry -> producer -> env_fingerprint
 ...
 public:
  ...
  void WriteBigEndian(Integer value) {
    ...
    for (std::size_t byte_index = sizeof(Integer); byte_index > 0; --byte_index) {
      const auto shift = static_cast<std::size_t>((byte_index - 1) * kByteBits);
      output_.put(static_cast<char>((remaining >> shift) & kByteMask));
    }
  }
...
void WriteInstruction(BytecodeWriter& writer, const Instruction& instruction) {
  ...
  writer.WriteBigEndian(static_cast<std::uint16_t>(*opcode));
...
// windows-arm32 序列化格式（大端）：
// [0..3]   4 字节魔数
// [4]      1 字节 byte_order
// [5..6]   2 字节 flags (big-endian)
// [7]      1 字节 platform_id
// [8..9]   2 字节 version (big-endian)
// [10..]   functions -> entry -> producer -> env_fingerprint
...
std::optional<BytecodeProgram> readBytecode(std::istream& input, const std::string& name, Diagnostics& diagnostics) {
  ...
  if (!matchesCurrentEnvironment(program.environmentFingerprint)) {
    AddBytecodeError(
        diagnostics,
        bytecode_diagnostic::kEnvironmentMismatch,
        name,
        "bytecode environment " + shortFingerprint(program.environmentFingerprint) +
