...
// linux_arm64 使用小端编码（Little-Endian），但字段顺序与现有 bytecode_binary.cc 不同。
// 字符串长度使用变长编码（varint）：每字节低 7 位为数据，最高位为 1 表示后续还有字节。
class BytecodeWriter {
 public:
  explicit BytecodeWriter(std::ostream& output) : output_(output) {}

  template <typename Integer>
  void WriteLittleEndian(Integer value) {
    ...
    for (std::size_t byte_index = 0; byte_index < sizeof(Integer); ++byte_index) {
      output_.put(static_cast<char>(remaining & kByteMask));
      remaining >>= kByteBits;
    }
  }
...
class BytecodeReader {
 public:
  ...
  template <typename Integer>
  std::optional<Integer> ReadLittleEndian(std::string_view field) {
    static_assert(std::is_integral_v<Integer>);
    using UnsignedInteger = std::make_unsigned_t<Integer>;
    ...
    for (std::size_t byte_index = 0; byte_index < sizeof(Integer); ++byte_index) {
      ...
      value |= static_cast<UnsignedInteger>(*byte) << (byte_index * kByteBits);
    }
    return static_cast<Integer>(value);
  }

  // 变长编码读取
  std::optional<std::uint32_t> ReadVarInt(std::string_view field) {
    ...
    Fail(bytecode_diagnostic::kStringTooLarge, std::string{"varint field '"} + std::string{field} + "' is too long");
    return std::nullopt;
  }

  std::optional<std::string> ReadString(std::string_view field) {
    const auto size = ReadVarInt(field);
    if (!size) {
      return std::nullopt;
    }
  ...
  bool ReadStringVector(std::vector<std::string>& output, std::string_view field) {
    const auto count = ReadVarInt(field);
    if (!count) {
      return false;
    }
 ...
 private:
  bool ReadRaw(char* data, std::size_t size) {
    ...
    return static_cast<bool>(input_.read(data, static_cast<std::streamsize>(size)));
  }
...
void WriteFunction(BytecodeWriter& writer, const FunctionBytecode& function) {
  ...
  writer.WriteVarInt(CheckedNonNegative(function.maxStackSize, kMaxStackLabel));
  writer.WriteVarInt(CheckedNonNegative(function.securityLevel, kSecurityLevelLabel));
  writer.WriteVarInt(function.returnable ? bytecode_format::kBooleanTrue : bytecode_format::kBooleanFalse);
  ...
  writer.WriteVarInt(CheckedCount(function.code.size(), kInstructionVectorLabel));
...
std::optional<Instruction> ReadInstruction(BytecodeReader& reader) {
  const auto raw_opcode = reader.ReadLittleEndian<std::uint16_t>(field_name::kOpcode);
  if (!raw_opcode) {
    return std::nullopt;
  }
...
std::optional<FunctionBytecode> ReadFunction(BytecodeReader& reader) {
  ...
  auto max_stack_size = reader.ReadVarInt(field_name::kFunctionMaxStack);
  auto security_level = reader.ReadVarInt(field_name::kFunctionSecurityLevel);
  auto returnable = reader.ReadVarInt(field_name::kFunctionReturnable);
  ...
  const auto instruction_count = reader.ReadVarInt(field_name::kInstructionCount);
...
// linux_arm64 字节码格式（小端编码，字段顺序与现有 bytecode_binary.cc 和 linux_arm32 不同）：
// [0..3]   4 字节魔数
// [4]      1 字节 platform id
// [5]      1 字节字节序标签（提前到 platform_id 之后）
// [6..7]   2 字节 version (little-endian)
// [8..9]   2 字节 flags (little-endian)
// [10..]   producer -> env_fingerprint -> entry -> functions
// 与现有格式差异：byte_order 提前，producer 在 env_fingerprint 之前，entry 在最后，
// 且字符串长度和计数使用变长编码（varint）而非固定 4 字节。
void writeBytecode(std::ostream& output, const BytecodeProgram& program) {
  ...
  writer.WriteVarInt(CheckedCount(program.functions.size(), kFunctionVectorLabel));
...
std::optional<BytecodeProgram> readBytecode(std::istream& input, const std::string& name, Diagnostics& diagnostics) {
  ...
  const auto function_count = reader.ReadVarInt(field_name::kFunctionCount);
  if (!function_count) {
    return std::nullopt;
  }
