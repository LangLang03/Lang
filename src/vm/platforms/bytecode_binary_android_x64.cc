...
// android-x64 序列化格式（大端编码）：
// [0..3]   4 字节魔数
// [4]      1 字节 platform id
// [5]      1 字节字节序标签
// [6..7]   2 字节 flags (big-endian)
// [8..9]   2 字节 version (big-endian)
// [10..]   environmentFingerprint / producer / entry / functions
// 注意：与现有 bytecode_binary.cc 和 android_x86 的字段顺序都不同
//       （byte_order 在 flags 之前，flags 在 version 之前，producer 在 entry 之前）。
...
std::optional<BytecodeProgram> readBytecode(std::istream& input, const std::string& name, Diagnostics& diagnostics) {
  ...
  // android-x64 字段顺序：platform_id -> byte_order -> flags -> version
  ...
  // android-x64 字段顺序：env_fingerprint -> producer -> entry -> functions
