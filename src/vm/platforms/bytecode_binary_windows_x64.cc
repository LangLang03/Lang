...
// windows_x64 函数字段顺序：name -> securityLevel -> maxStackSize -> returnable -> ...
...
std::optional<FunctionBytecode> ReadFunction(BytecodeReader& reader) {
  ...
  if (!name || !security_level || !max_stack_size || !returnable || !category || !hash || !grantor) {
    return std::nullopt;
  }
...
// windows_x64 序列化格式（大端编码）：
// 头部字段顺序：magic(4) -> version(2) -> platform_id(1) -> flags(2) -> byte_order(1)
//               -> env_fingerprint -> entry -> producer -> functions
