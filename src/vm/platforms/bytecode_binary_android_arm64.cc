...
// android_arm64 使用小端编码，但字段顺序与现有 bytecode_binary.cc 不同。
...
// android_arm64 序列化格式 (小端编码，字段顺序与现有格式不同):
// [0..3]   4 字节魔数
// [4]      1 字节字节序标签
// [5]      1 字节 platform id
// [6..7]   2 字节 flags (little-endian)
// [8..9]   2 字节 version (little-endian)
// [10..]   entry -> producer -> env_fingerprint -> functions
...
std::optional<BytecodeProgram> readBytecode(std::istream& input, const std::string& name, Diagnostics& diagnostics) {
  ...
  // android_arm64: entry -> producer -> env_fingerprint -> functions
